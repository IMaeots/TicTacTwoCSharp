...
{
    public static string? ValidateInputAsAlphanumeric(string name) =>
        IsAlphanumericRegex().IsMatch(name)
            ? null
            : "Input must contain only normal characters or numbers.";
    ...
    public static string? ValidateWinCondition(int winCondition, int gridHeight, int gridWidth) =>
        winCondition <= 0
            ...
            : winCondition > gridHeight && winCondition > gridWidth
                ? "Win condition cannot be greater than the grid's dimensions."
                : null;
    ...
    public static string? ValidateGridWidth(int gridWidth, int boardWidth) =>
        gridWidth > boardWidth
            ? "Grid width cannot be greater than the board width."
            : ValidateRangeAccordingToRules(gridWidth);

    public static string? ValidateGridHeight(int gridHeight, int boardHeight) =>
        gridHeight > boardHeight
            ? "Grid height cannot be greater than the board height."
            : ValidateRangeAccordingToRules(gridHeight);
    ...
    private static string? ValidateRangeAccordingToRules(int value)
    {
        return value is < MinimumGameDimension or > MaximumGameDimension
            ? $"Value must be between {MinimumGameDimension}-{MaximumGameDimension}."
            : null;
    }
    ...
    private static string? ValidateStartingPosition(int position, int boardDimension, int gridDimension, string axis)
    {
        ...
        return position < 0 || position > validLastPosition
            ? $"{axis} position must be between 0 and {validLastPosition}."
            : null;
    }
}
